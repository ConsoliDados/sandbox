...
3. **Discovery via glob.** We search the project root for `**/compose*.y{,a}ml` (matches `docker-compose.yml`, `compose.yaml`, `compose.dev.yml`, `services/compose.yml`, …). Multi-match: if more than one is found, error with the list and require `--compose-file PATH` to disambiguate. Single match wins automatically. `--compose-file PATH` always overrides discovery.
